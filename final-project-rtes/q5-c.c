...
void rgb2gray(int r, int g, int b, unsigned char *gray)
{
   int y;

   // replaces floating point coefficients with scaled 256 multiplier
   int c = 77, d = 151, e = 28;       
...
unsigned char bigbuffer_gray[(1280*960)];

static void process_image(const void *p, int size)
{
    int i, newi, grayi, newsize=0;
    ...
    else if(fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_YUYV)
    {
        ...
        //
        for(i=0, newi=0, grayi=0; i<size; i=i+4, newi=newi+6, grayi=grayi+2)
        {
            y_temp=(int)pptr[i]; u_temp=(int)pptr[i+1]; y2_temp=(int)pptr[i+2]; v_temp=(int)pptr[i+3];
            yuv2rgb(y_temp, u_temp, v_temp, &bigbuffer[newi], &bigbuffer[newi+1], &bigbuffer[newi+2]);
            yuv2rgb(y2_temp, u_temp, v_temp, &bigbuffer[newi+3], &bigbuffer[newi+4], &bigbuffer[newi+5]);
#if defined(ENABLE_TRANSFORM)       
            rgb2gray(bigbuffer[newi], bigbuffer[newi+1], bigbuffer[newi+2], &bigbuffer_gray[grayi]);
            rgb2gray(bigbuffer[newi+3], bigbuffer[newi+4], bigbuffer[newi+5], &bigbuffer_gray[grayi+1]);
#endif
        }
        ...
        if(framecnt > -1) 
        {
            ...
            dump_pgm(bigbuffer_gray, ((size*2)/4), framecnt, &frame_time);    //grayscale buffer
            syslog(LOG_INFO, "Dump Transform RGB to Gray of size %d\n", size);
#endif
        }
#else
...
static void *mainloop(void *thread_para) {
    //unsigned int count;
    ...
    threadParams_t *threadParams = (threadParams_t *)thread_para;
    int tid = threadParams->threadIdx;
    ...
    read_delay.tv_sec  = (int)(1/FRAME_RATE_SET);
    if(read_delay.tv_sec > 0)
        read_delay.tv_nsec = (int)(((1.0/(double) FRAME_RATE_SET) - 1.0) * (double) NSEC_PER_SEC);
    else
        read_delay.tv_nsec = (int)((1.0/(double) FRAME_RATE_SET) * (double) NSEC_PER_SEC);
    //printf("Set Frame rate is %d\n", FRAME_RATE_SET);
    //printf("read_delay.tv_sec=%ld, read_delay.tv_nsec=%ld\n", read_delay.tv_sec, read_delay.tv_nsec);
    //count = frame_count;
    ...
    while (frame_count > 0)
    {
        for (;;)
        {
            ...
            /* start time stamp */ 
            clock_gettime(MY_CLOCK, &prev_time);
            ...
            if (read_frame())
            {
                ...
                clock_gettime(MY_CLOCK, &stop_time);
                ...
                delta_t(&stop_time, &prev_time, &delay_time);

                // prev_time.tv_nsec = stop_time.tv_nsec;
                // prev_time.tv_sec = stop_time.tv_sec;
                
                printf("Frame rate got for %d is %f fps\n", tid, 
                                    (1.0/((double)(delay_time.tv_sec) + ((double)(delay_time.tv_nsec) / NSEC_PER_SEC))));
                break;
            }
...
{
    openlog("Simple-Capture-1800 Log", LOG_PID|LOG_CONS, LOG_USER);
